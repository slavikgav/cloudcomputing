...
function enableButtons () {
	...
	btn = document.getElementById("input_greet_by_duration");
	btn.onclick=function(){greetByDuration();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a personal greeting with duration";
}
...
function greetByDuration(){
	var name = document.getElementById("name_field").value;
	var duration = parseInt(document.getElementById("duration_field").value); 
	var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'duration':duration});
	request.execute(sayHelloCallback);
}
